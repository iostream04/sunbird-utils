...
public class EncryptionDecryptionServiceTest {
  ...
  public void testDataDecryptionFrMap() {
    try {
      assertEquals(decryptionService.decryptData(encryptionService.encryptData(map2)).get(JsonKey.FIRST_NAME), "Amit");
    } catch (Exception e) {
    }
  }
  ...
  public void testDataDecryptionFrMapWithNullValue() {
    try {
      ...
      assertEquals(decryptionService.decryptData(encryptionService.encryptData(map2)).get(JsonKey.LOCATION), null);
    } catch (Exception e) {
    }
  }
  ...
  public void testDataDecryptionFrMapWithEmptyValue() {
    try {
      ...
      assertEquals(decryptionService.decryptData(encryptionService.encryptData(map2)).get(JsonKey.LOCATION), "");
    } catch (Exception e) {
    }
  }
  ...
  public void testDataDecryptionFrMapWithMapList() {
    try {
      map2.put(JsonKey.LOCATION, "");
      assertEquals(decryptionService.decryptData(encryptionService.encryptData(mapList2)).get(0).get(JsonKey.LOCATION), "");
    } catch (Exception e) {
    }
  }
  ...
  public void testDataEncryptionFrListMap() {
    try {
      if(JsonKey.ON.equalsIgnoreCase(sunbirdEncryption)){
      assertNotEquals(encryptionService.encryptData(mapList).get(0).get(JsonKey.FIRST_NAME), "Amit");
      }
    } catch (Exception e) {
    }
  }
  ...
  public void testBDataDecryption() {
    try {
      if(JsonKey.ON.equalsIgnoreCase(sunbirdEncryption)){
       assertNotEquals(encryptionService.encryptData("Hello"), decryptionService.decryptData(encryptionService.encryptData("Hello")));
      }
      } catch (Exception e) {
    }
  }
