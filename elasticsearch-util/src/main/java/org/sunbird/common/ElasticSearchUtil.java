...
public class ElasticSearchUtil {
  ...
  public static Map<String, Object> searchData(String index, String type,
    ...
    try {
      verifyOrCreateIndexAndType(index, type);
      sr = ConnectionManager.getClient()
          .search(new SearchRequest(index).types(type).source(sourceBuilder)).get();
      ...
      if (!typeMap.containsKey(type)) {
        ...
        try {
          ...
          if (typeResponse) {
            ...
            boolean response = addOrUpdateMapping(indices, type,
  ...
  public static Response searchMetricsData(String index, String type, String rawQuery) {
    ...
    String baseUrl = "http://" + ConfigUtil.config.getStringList(JsonKey.SUNBIRD_ES_IP) + ":" + ConfigUtil.config.getString(JsonKey.ES_METRICS_PORT);
